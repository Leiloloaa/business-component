<!-- 水平 bar 组件 -->
<template>
  <div class="bar-wrap" ref="barWrapRef" :class="TOOL_countryCode">
    <!-- 进度条 绝对定位 -->
    <div v-bg="`p1-bar`" class="bar-bg">
      <div class="bar-inner">
      ...
      <div class="bar-lv-list">
        <template v-for="(item, idx) in taskList">
          <div class="bar-lv-info" :style="getItemStyle(idx)">
            <div
              class="icon"
              :class="{ active: groupInfo.curIdx === idx }"
              v-bg="currentLevel > idx ? `bar-icon-act` : `bar-icon`"
            ></div>

            <div class="text fc">
              <Outline
                class="ov"
                :color="currentLevel > idx ? '0.05rem #FFF0AC' : '0.05rem #FFF0AC'"
...
const groupInfo: any = inject('groupInfo', { curIdx: 0 })
...
/** 节点定位样式：按 left/right + translateX 居中，兼容 RTL */
const getItemStyle = (idx: number) => {
  const offset = `${segmentEndPositions.value[idx] ?? 0}rem`
  const isRtl = barDirection.value === 'rtl'
  return {
